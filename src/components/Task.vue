  ...
  <div @dblclick="$emit('toggle-reminder', task.id)" :class="[task.reminder ? 'reminder' : '', 'task']">
    <div class="task__row">
      ...
      <div class="task__buttons">
        ...
        <button @click="$emit('delete-task', task.id)" name="button" class="delete-button"></button>
      </div>
    </div>
    <div>
      <input
...
export default {
  ...
  methods: {
    changeText() {
      this.$emit('change-text', this.task.id, this.newText, this.newDate)
